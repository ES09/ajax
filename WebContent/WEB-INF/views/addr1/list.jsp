...
<script>
	...
	function search(){
		...
		location.href="/addr/list?page=${page}&pageCount=${pageCount}&ad_dong=" + ad_dong;
	}
</script>
<label for="ad_dong">읍·면·동 : </label>
<input type="text" name="ad_dong" id="ad_dong" value="${param.ad_dong}">
<button onclick="search()">검색</button>
<select name="pageCount" onchange="changePageCount(this)">
	<option value="10" <c:if test="${pageCount==10}">selected</c:if> >10개씩 보기</option>
	<option value="20" <c:if test="${pageCount==20}">selected</c:if> >20개씩 보기</option>
	<option value="30" <c:if test="${pageCount==30}">selected</c:if> >30개씩 보기</option>
	<option value="40" <c:if test="${pageCount==40}">selected</c:if> >40개씩 보기</option>
	<option value="50" <c:if test="${pageCount==50}">selected</c:if> >50개씩 보기</option>
</select>
...
<table border="1">
	...
	<tr>
		...
		<td>${addr.ad_sido}</td>
		<td>${addr.ad_gugun}</td>
		<td><a href="/addr/view?ad_num=${addr.ad_num}&page=${1}&pageCount=${pageCount}&ad_dong=${param.ad_dong}">${addr.ad_dong}</a></td>
		<td>${addr.ad_lee}</td>
		<td>${addr.ad_bunji}</td>
		<td>${addr.ad_ho}</td>
	</tr>
