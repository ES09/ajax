...
public class AddrDAOImpl implements AddrDAO {
	...
	private static String selectAddrListSql = "select * from (select rownum as rown, addr.* from "  
										+ " (select * from address $where$ order by ad_num) "
	...
	public List<Map<String, String>> selectAddrList(Map<String, String> addr) {
		...
		try {
			...
			ps.setString(1, addr.get("lNum")); // 끝 값
			ps.setString(2, addr.get("sNum")); // 시작 값
			if(adDong!=null) {
